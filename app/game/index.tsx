...
export default function GameScreen() {
  const { players, playerTurn, updateGameBoard, changePlayerTurn } =
    useGameBoard();
  ...
  const currentPlayerName =
    players[playerTurn]?.name || `Player ${playerTurn + 1}`;
  ...
  const [message, setMessage] = useState(
    `C'est à ${currentPlayerName} de jouer`,
  );
  ...
  const handleFire = (): void => {
    ...
    if (cellValue === 2) {
      updateGameBoard(x, y);
      setMessage(
        `${currentPlayerName} a touché un bateau de ${opponentName} !`,
      );
